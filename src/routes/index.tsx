...
const getTodos = createServerFn().handler(async () => {
  const todos = await serverRuntime.runPromiseExit(
    Effect.gen(function* () {
      ...
      yield* Effect.log(`[getTodos] Session result: ${session ? "authenticated" : "not authenticated"}`);
      
      if (!session?.user?.id) {
        yield* Effect.log("[getTodos] No authenticated user, returning empty array");
        return [];
      }
      ...
      yield* Effect.log(`[getTodos] Successfully fetched ${userTodos.length} todos`);

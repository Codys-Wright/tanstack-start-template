...
export function randomizeThemeParams(
  ...
  const availableFonts = applyBias(
    FONTS,
    context,
    RANDOMIZE_BIASES.fonts
  )
  context.font =
    lockedParams.font || randomItem(availableFonts).value

  // Randomize radius (unless locked)
  const availableRadii = applyBias(
    RADII,
    context,
    RANDOMIZE_BIASES.radius
  )

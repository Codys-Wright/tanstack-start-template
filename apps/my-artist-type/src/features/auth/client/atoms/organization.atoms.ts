 ...
 */
class OrganizationApi extends Effect.Service<OrganizationApi>()("@features/auth/OrganizationApi", {
	effect: Effect.sync(() => ({
		...
		leaveOrganization: (organizationId: string) =>
			Effect.tryPromise({
				try: async () => {
					...
					new Error(
						`Failed to leave organization: ${error instanceof Error ? error.message : String(error)}`,
					),
			}),
		...
		setActiveTeam: (teamId: string | null) =>
			Effect.tryPromise({
				try: async () => {
					...
					new Error(
						`Failed to set active team: ${error instanceof Error ? error.message : String(error)}`,
					),
			}),
		...
		listOrganizationMembers: (organizationId?: string) =>
			Effect.tryPromise({
				try: async () => {
					...
					new Error(
						`Failed to list organization members: ${error instanceof Error ? error.message : String(error)}`,
					),
			}),
	})),
}) {}
 ...
 */
export const updateOrganizationAtom = orgRuntime.fn<{ organizationId: string; data: any }>()(
	Effect.fnUntraced(function* (input, get) {
		const api = yield* OrganizationApi;
 ...
 */
export const deleteOrganizationAtom = orgRuntime.fn<{ organizationId: string }>()(
	Effect.fnUntraced(function* (input, get) {
		const api = yield* OrganizationApi;
 ...
 */
export const setActiveOrganizationAtom = orgRuntime.fn<{ organizationId: string | null }>()(
 ...
 */
export const leaveOrganizationAtom = orgRuntime.fn<{ organizationId: string }>()(
	Effect.fnUntraced(function* (input, get) {
		const api = yield* OrganizationApi;
 ...
 */
export const removeMemberAtom = orgRuntime.fn<{ memberIdOrEmail: string; organizationId?: string }>()(
	Effect.fnUntraced(function* (input) {
		const api = yield* OrganizationApi;
 ...
 */
export const removeTeamAtom = orgRuntime.fn<{ teamId: string; organizationId?: string }>()(
	Effect.fnUntraced(function* (input) {
		const api = yield* OrganizationApi;
 ...
 */
export const removeTeamMemberAtom = orgRuntime.fn<{ teamId: string; userId: string }>()(
	Effect.fnUntraced(function* (input) {
		const api = yield* OrganizationApi;

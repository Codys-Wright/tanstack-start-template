...
export const TodosApiLive = HttpApiBuilder.group(DomainApi, 'todos', (handlers) =>
  handlers
    .handle('list', () =>
      Effect.gen(function* () {
        const currentUser = yield* AuthContext;
        yield* Effect.log(`[HTTP API] Listing todos for user: ${currentUser.userId}`);
    ...
    .handle('getById', ({ path }) =>
      Effect.gen(function* () {
        const currentUser = yield* AuthContext;
        yield* Effect.log(`[HTTP API] Getting todo ${path.id} for user: ${currentUser.userId}`);
        const todos = yield* TodosService;
    ...
    .handle('create', ({ payload }) =>
      Effect.gen(function* () {
        const currentUser = yield* AuthContext;
        yield* Effect.log(`[HTTP API] Creating todo for user: ${currentUser.userId}`);
        const todos = yield* TodosService;
    ...
    .handle('update', ({ path, payload }) =>
      Effect.gen(function* () {
        const currentUser = yield* AuthContext;
        yield* Effect.log(`[HTTP API] Updating todo ${path.id} for user: ${currentUser.userId}`);
        const todos = yield* TodosService;
    ...
    .handle('remove', ({ path }) =>
      Effect.gen(function* () {
        const currentUser = yield* AuthContext;
        yield* Effect.log(`[HTTP API] Removing todo ${path.id} for user: ${currentUser.userId}`);
        const todos = yield* TodosService;

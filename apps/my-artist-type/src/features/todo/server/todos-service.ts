...
import { CreateTodoInput, TodoId, TodoNotFound, UpdateTodoInput } from '../domain/todo-schema.js';
  ...
  effect: Effect.gen(function* () {
    ...
    const getById = (id: TodoId, userId: UserId) =>
      repo.findById({ id, userId }).pipe(
        ...
        Effect.tapError(() => Effect.log(`[TodosService.getById] Todo not found: ${id}`)),
      );
    ...
    const update = (id: TodoId, input: UpdateTodoInput, userId: UserId) =>
      Effect.gen(function* () {
        ...
        Effect.mapError(() => new TodoNotFound({ id })),
        Effect.tapError(() => Effect.log(`[TodosService.update] Todo not found: ${id}`)),
      );
    ...
    const remove = (id: TodoId, userId: UserId) =>
      Effect.gen(function* () {
        ...
        Effect.mapError(() => new TodoNotFound({ id })),
        Effect.tapError(() => Effect.log(`[TodosService.remove] Todo not found: ${id}`)),
      );
